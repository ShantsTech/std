...
const hideTooltip = () => {
  document.getElementById('std-pie-chart-tooltip')?.remove();
};

export const OxdPieChartTooltip: Plugin = {
  id: 'stdPieChartTooltip',
  afterDatasetsDraw(chart: Chart<'pie'>) {
    ...
    if (activeElements.length === 0) {
      ...
      const tooltipEl =
        document.getElementById('std-pie-chart-tooltip') ||
      ...
      tooltipEl.id = 'std-pie-chart-tooltip';
      tooltipEl.classList.add('std-pie-chart-tooltip');

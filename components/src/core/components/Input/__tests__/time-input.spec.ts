...
describe('TimeInput.vue', () => {
  ...
  it('should open timepicker on click', async () => {
    ...
    expect(wrapper.find('.std-time-picker').exists()).toBeTruthy();
  });
  it('should not open timepicker on click, if disabled', async () => {
    ...
    expect(wrapper.find('.std-time-picker').exists()).toBeFalsy();
  });
  ...
  it('should increment/decrement hour', async () => {
    ...
    await wrapper.vm.$nextTick();
    const picker = wrapper.findComponent(TimePicker);
    const incrementBtn = picker.find('.std-time-hour-input-up');
    const decrementBtn = picker.find('.std-time-hour-input-down');
    expect(wrapper.emitted('update:modelValue')[0]).toEqual(['01:00']);
  ...
  it('should increment/decrement minute', async () => {
    ...
    await wrapper.vm.$nextTick();
    const picker = wrapper.findComponent(TimePicker);
    const incrementBtn = picker.find('.std-time-minute-input-up');
    const decrementBtn = picker.find('.std-time-minute-input-down');
    expect(wrapper.emitted('update:modelValue')[0]).toEqual(['01:00']);
    await incrementBtn.trigger('click');

...
describe('Calendar.vue', () => {
  ...
  it('should select date on click', async () => {
    ...
    expect(date.find('.std-calendar-date.--selected').exists()).toBeTruthy();
  });
  it('should show today date', async () => {
    const wrapper = mount(Calendar, {});
    expect(wrapper.find('.std-calendar-date.--today').text()).toStrictEqual(
  ...
  it('should show attributes in calendar', async () => {
    ...
    expect(
      date[nonWorkingDay.getDate()]
        .find('.std-calendar-date-wrapper.--non-working-day')
        .exists(),
    ).toBeTruthy();
    expect(
      date[halfWorkingDay.getDate()]
        .find('.std-calendar-date-wrapper.--working-day-half')
        .exists(),
    ).toBeTruthy();
  });
  it('should show events in calendar', async () => {
    ...
    expect(
      date[event1.date.getDate()]
        .find('.std-calendar-date.--holiday-full')
        .exists(),
    ).toBeTruthy();
    expect(
      date[event2.date.getDate()]
        .find('.std-calendar-date.--holiday-half')
        .exists(),
    ).toBeTruthy();
  });

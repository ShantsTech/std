...
describe('CalendarController.vue', () => {
  ...
  it('renders STD CalendarController', () => {
  ...
  it('should open month dropdown on click', async () => {
    ...
    await wrapper.find('.std-calendar-selector-month').trigger('click');
    expect(
      wrapper.findAll(
        '.std-calendar-selector-month .std-calendar-dropdown--option',
      ).length,
    ).toEqual(12);
  });
  it('should open year dropdown on click', async () => {
    ...
    await wrapper.find('.std-calendar-selector-year').trigger('click');
    expect(
      wrapper.findAll(
        '.std-calendar-selector-year .std-calendar-dropdown--option',
      ).length,
    ).toEqual(4);
  });
  it('should select month on click', async () => {
    ...
    await wrapper.find('.std-calendar-selector-month').trigger('click');
    await wrapper
      .find('.std-calendar-selector-month .std-calendar-dropdown--option')
      .trigger('click');
  ...
  it('should select year on click', async () => {
    ...
    await wrapper.find('.std-calendar-selector-year').trigger('click');
    await wrapper
      .find('.std-calendar-selector-year .std-calendar-dropdown--option')
      .trigger('click');
    expect(wrapper.emitted('update:modelValue')).toEqual([
      [{month: 2, year: 2019}],
    ]);

...
describe('TabContainer.vue', () => {
  ...
  it('should render STD Tab Container with tabs', async () => {
    const wrapper = mount(TabContainer, {
      slots: {
        default: () => Tabs,
      },
    });
    const tabs = wrapper.findAll('.std-tab-segment');
    expect(tabs.length).toBe(3);
    expect(tabs[0].classes()).toContain('std-tab-segment--active');
  });
  ...
  it('should select current tab by default', async () => {
    ...
    const tabs = wrapper.findAll('.std-tab-segment');
    expect(tabs[1].classes()).toContain('std-tab-segment--active');
  });
